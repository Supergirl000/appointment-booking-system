<x-app-layout>
    <x-slot name="header">
        <x-page-header
            ...
            :subtitle="__('Review customer contact details and appointment history.')"
        >
            <x-slot name="actions">
        ...
        <div class="space-y-6">
            <section class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
                <dl class="grid gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        <dt class="text-sm font-medium text-slate-500">{{ __('Phone') }}</dt>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-base font-semibold text-slate-950">{{ $customer->created_at?->translatedFormat('M d, Y') }}</dd>
                    </div>
                    <div class="sm:col-span-2">
                        <dt class="text-sm font-medium text-slate-500">{{ __('Notes') }}</dt>
            ...
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div class="mb-5">
                    <h2 class="text-lg font-bold text-slate-950">{{ __('Appointment History') }}</h2>
                    <p class="mt-1 text-sm text-slate-500">{{ __('Past and upcoming appointments for this customer will appear here in a later phase.') }}</p>
                </div>

                <x-empty-state
                    :title="__('No appointment history yet')"
                    :description="__('The appointments module has not been built yet. Once it is available, this section will show this customer\'s booking history.')"
                />
            </section>
        </div>

        <aside class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="text-lg font-bold text-slate-950">{{ __('Customer Actions') }}</h2>
