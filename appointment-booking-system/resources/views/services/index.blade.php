<x-app-layout>
    <x-slot name="header">
        <x-page-header
            title="Services"
            subtitle="Manage the bookable salon and spa services offered to customers."
        >
            <x-slot name="actions">
                <x-primary-button :href="route('services.create')">Create Service</x-primary-button>
            </x-slot>
        </x-page-header>
    </x-slot>
    ...
    <div class="space-y-5">
        ...
        <x-table-card padded>
            <form method="GET" action="{{ route('services.index') }}" class="grid gap-4 lg:grid-cols-[1fr_220px_auto]">
                <x-form-input label="Search" name="search" type="search" :value="$search" placeholder="Search by service name or category" />
                ...
                <div class="flex flex-col gap-2 self-end sm:flex-row">
                    ...
                    <x-secondary-button :href="route('services.index')">Reset</x-secondary-button>
                </div>
            </form>
        </x-table-card>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th scope="col" class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Duration</th>
                                <th scope="col" class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Price</th>
                                ...
                                <tr class="hover:bg-slate-50/70">
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex flex-wrap items-center justify-end gap-2">
                                            <a href="{{ route('services.show', $service) }}" class="rounded-md px-2.5 py-1.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-100 hover:text-slate-950">View</a>
                                            <a href="{{ route('services.edit', $service) }}" class="rounded-md px-2.5 py-1.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-100 hover:text-slate-950">Edit</a>
            ...
            @else
                <div class="p-6">
                    <x-empty-state
                        ...
                        description="Create your first service or adjust the search filters to find existing services."
                        action-text="Create Service"
