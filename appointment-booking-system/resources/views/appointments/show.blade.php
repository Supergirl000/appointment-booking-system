<x-app-layout>
    <x-slot name="header">
        <x-page-header
            ...
            :subtitle="__('Review the customer, service, staff assignment, schedule, and current status.')"
        >
            <x-slot name="actions">
    ...
    <div class="grid gap-6 xl:grid-cols-[1fr_320px]">
        <section class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <dl class="grid gap-6 sm:grid-cols-2">
                ...
                <div>
                    <dt class="text-sm font-medium text-slate-500">{{ __('Service') }}</dt>
                    <dd class="mt-1 text-base font-semibold text-slate-950">{{ $appointment->service->name }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-slate-500">{{ __('Staff') }}</dt>
                ...
                <div>
                    <dt class="text-sm font-medium text-slate-500">{{ __('Date') }}</dt>
                    <dd class="mt-1 text-base font-semibold text-slate-950">{{ $appointment->appointment_date->translatedFormat('M d, Y') }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-slate-500">{{ __('Time') }}</dt>
        ...
        <aside class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="text-lg font-bold text-slate-950">{{ __('Appointment Actions') }}</h2>
            <div class="mt-5 space-y-3">
                <a href="{{ route('appointments.index') }}" class="flex items-center justify-between rounded-lg border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-950">
                    <span>{{ __('Back to Appointments') }}</span>
                ...
                <form method="POST" action="{{ route('appointments.destroy', $appointment) }}" onsubmit="return confirm('Delete this appointment?');">
                    ...
                    <button type="submit" class="flex w-full items-center justify-between rounded-lg border border-rose-200 px-4 py-3 text-sm font-semibold text-rose-600 transition hover:bg-rose-50">
                        <span>{{ __('Delete Appointment') }}</span>
                        <span>#</span>
                    </button>
                </form>
            </div>
